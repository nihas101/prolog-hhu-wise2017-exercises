...
phi_(0,0) :- !.
phi_(1,0) :- !.
phi_(M,N) :- range_1(M,L) ,
             phi_(M,L,N) , !.
phi_(_,[],0) :- !.
phi_(M,[H|T],N) :- coprime(M,H) ,
                   phi_(M,T,N1) ,
                   N is N1+1 , !.
...
duplicate_([],_,[]-[]) :- !.
duplicate_([H|T],N,R-R1) :- duplicate_Element(H,N,R-L2) ,
                            duplicate_(T,N,L2-R1) , !.

duplicate_Element(_,0,T-T) :- !.
duplicate_Element(H,N,[H|T]-T1) :- N1 is N-1 ,
                                   duplicate_Element(H,N1,T-T1) , !.
